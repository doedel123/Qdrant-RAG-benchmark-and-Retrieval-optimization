#!/usr/bin/env python3
"""
Reindex Fachliteratur mit MixedBread/deepset DE-Embeddings
==========================================================
Liest alle Chunks aus der Qdrant-Collection ``fachliteratur`` (E5-Vektoren),
embedded sie mit ``mixedbread-ai/deepset-mxbai-embed-de-large-v1`` (lokal,
SentenceTransformer) und schreibt sie in eine neue Collection
``fachliteratur_mxbai``. BM25-Sparse-Vektoren werden aus dem Text neu
berechnet. Payload und IDs bleiben identisch.

Usage:
    python reindex_mxbai.py               # legt fachliteratur_mxbai an
    python reindex_mxbai.py --recreate    # loescht bestehende Collection und neu
"""
...
DST_COLLECTION = "fachliteratur_mxbai"
...
def main():
    ...
    total = qc.get_collection(SRC_COLLECTION).points_count
    print(f"Quelle: '{SRC_COLLECTION}' mit {total} Punkten")

    print(f"Lade Modell '{MODEL_NAME}' ...")
    model = SentenceTransformer(MODEL_NAME)
    print(f"Modell geladen (Device: {model.device}, Dim: {model.get_sentence_embedding_dimension()})")
    ...
    def flush():
        ...
        # Passagen: kein Prompt-Prefix, nur der Text
        embs = model.encode(
            texts,
    ...
    with tqdm(total=total, desc="Reindex") as bar:
        for pid, text, payload in iter_source(qc):
            ...
            if len(buffer) >= EMBED_BATCH * 4:  # groessere Flush-Batches fuer Upload
                flush()
                bar.update(EMBED_BATCH * 4)
        if buffer:
